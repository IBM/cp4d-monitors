...
cognos_analytic_db_username='cognos_analytic_db_username'
cognos_analytic_db_password='cognos_analytic_db_password'
...
def get_service_instances(instance_type):
    cognos_instances = []
    for instance in service_instaces['service_instances']:
        if instance['addon_type'] == instance_type:
...
def get_all_services_instances():
    bearer_token=get_admin_token()
    headers = {'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': str(bearer_token)}
    service_instances_api_url='/zen-data/v3/service_instances?fetch_all_instances=true'
    res=requests.get(cp4d_host+service_instances_api_url,verify=False, headers=headers)
    if res.status_code!=200:
        print('Error requesting all services instances - status_code - ' + str(res.status_code))
        try:
            print(res.json())
        except Exception:
            print(res.text)
