...
def get_cp4d_deployments(space_id):
    bearer_token=get_admin_token()
    headers = {'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': str(bearer_token)}
...
def get_deployment_jobs(space_id):
    bearer_token=get_admin_token()
    headers = {'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': str(bearer_token)}
    jobs_api_url='/v2/jobs/?version=2020-09-01&&space_id={}'.format(space_id)
    res=requests.get(cp4d_host+jobs_api_url,verify=False, headers=headers)
    jobs={}
    if res.status_code == 200:
...
def get_space_job_runs(space_id,job_id):
    bearer_token=get_admin_token()
    headers = {'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': str(bearer_token)}
    runs_api_url=f'/v2/jobs/{job_id}/runs?space_id={space_id}'
    res=requests.get(cp4d_host+runs_api_url,verify=False, headers=headers)
    jobs={}
    if res.status_code == 200:
        jobs=json.loads(res.text)
    else:
        print('Error requesting get all runs - status_code - ' + str(res.status_code))
        try:
            print(res.json())
        except Exception:
            print(res.text)
